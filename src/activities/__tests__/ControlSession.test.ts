...
describe("ControlSession", () => {
    describe("execute", () => {
        ...
        it("calls the API using POST", async () => {
            ...
            mockPost.mockImplementationOnce((service: ApiService, path: string,  params?: Record<string, any>, data?: Record<string, any>) => {
                expect(service).toBe(inputs.service);
                expect(path).toBe(`sessions/${inputs.path}`);
                expect(JSON.stringify(params)).toBe(JSON.stringify({action: inputs.action}));
