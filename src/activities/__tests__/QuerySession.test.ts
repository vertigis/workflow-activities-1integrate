...
describe("QuerySession", () => {
    describe("execute", () => {
        ...
        it("calls the API using POST", async () => {
            ...
            mockGet.mockImplementationOnce((service: ApiService, path: string,  params?: Record<string, any>) => {
                expect(service).toBe(inputs.service);
                expect(path).toBe(`results/${inputs.path}`);
                expect(JSON.stringify(params)).toBe(JSON.stringify({detail: inputs.detail}));
